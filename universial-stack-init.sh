...
read -rp "Vollsnapshots aufbewahren für [30] Tage: " SNAPSHOT_KEEP_DAYS
...
RUN apk add --no-cache rclone openrc mysql-client openssh-client postgresql-client curl dcron
...
export RCLONE_CONFIG_STORAGEBOXBASE_HOST=${STORAGEBOX_HOST}
export RCLONE_CONFIG_STORAGEBOXBASE_USER=${STORAGEBOX_USER}
export RCLONE_CONFIG_STORAGEBOXBASE_PASS="$(rclone obscure "${STORAGEBOX_PASSWORD}")"
export RCLONE_CONFIG_STORAGEBOXBASE_PORT=23
export RCLONE_CONFIG_STORAGEBOXBASE_USE_SSH_AGENT=false
export RCLONE_CONFIG_STORAGEBOXBASE_HOST_KEY_CHECKING=false

# 2. Verschlüsselte Schicht (Name: StorageBox)
export RCLONE_CONFIG_STORAGEBOX_TYPE=crypt
export RCLONE_CONFIG_STORAGEBOX_REMOTE=StorageBoxBase:${STORAGEBOX_PATH}
...
echo ""
echo "--- Zugangsdaten ---"
echo "🌐 Traefik Dashboard: https://${TRAEFIK_HOST} (BasicAuth: ${TFA_USER}/<dein-Passwort>)"
